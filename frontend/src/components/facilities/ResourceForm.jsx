...
export default function ResourceForm({ initial, onSubmit, onClose }) {
  ...
  // ✅ Handle input change + clear hours when EQUIPMENT
  const set = (e) => {
    ...
    setForm((f) => {
      ...
      // 🔥 If switching to EQUIPMENT → clear hours + capacity
      if (name === 'type' && value === 'EQUIPMENT') {
        updated.capacity = '';
        updated.availabilityStart = '';
        updated.availabilityEnd = '';
      }
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      ...
      availabilityStart: form.type === 'EQUIPMENT' ? null : form.availabilityStart,
      availabilityEnd: form.type === 'EQUIPMENT' ? null : form.availabilityEnd,
    });
  };
        ...
        <div style={{ padding: '24px' }}>
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              {errors.name && <p style={{ fontSize: '11px', color: '#e03131' }}>{errors.name}</p>}
            </div>
            ...
            {form.type !== 'EQUIPMENT' && (
              <div style={{ marginBottom: '16px' }}>
                ...
                />
                {errors.capacity && <p style={{ fontSize: '11px', color: '#e03131' }}>{errors.capacity}</p>}
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
              <div>
                ...
                {errors.building && <p style={{ fontSize: '11px', color: '#e03131' }}>{errors.building}</p>}
              </div>
              <div>
                <label style={lbl}>Location *</label>
                <input name="location" value={form.location} onChange={set} style={inp(errors.location)} />
                {errors.location && <p style={{ fontSize: '11px', color: '#e03131' }}>{errors.location}</p>}
              </div>
            </div>
